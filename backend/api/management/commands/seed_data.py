...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not User.objects.filter(username='cashier').exists():
            u = User(username='cashier', email='cashier@shopease.com', role='cashier',
                     first_name='John', last_name='Cashier')
            u.set_password('cashier123')
            u.save()

        # Categories
        cats = ['Electronics', 'Groceries', 'Beverages', 'Snacks', 'Personal Care',
                'Dairy', 'Bakery', 'Household', 'Stationery', 'Clothing']
        cat_objs = {}
        for c in cats:
        ...
        # Suppliers
        suppliers_data = [
            ('TechWorld Distributors', '+91 98001 11111', 'techworld@email.com'),
            ('FreshFarm Supplies', '+91 98002 22222', 'freshfarm@email.com'),
            ('Metro Wholesale', '+91 98003 33333', 'metro@email.com'),
            ('National Distributors', '+91 98004 44444', 'national@email.com'),
        ]
        sup_objs = []
        for name, phone, email in suppliers_data:
            obj, _ = Supplier.objects.get_or_create(name=name, defaults={'phone': phone, 'email': email})
            sup_objs.append(obj)
        ...
        for name, mobile, email, credit in customers_data:
            obj, _ = Customer.objects.get_or_create(
                name=name,
                defaults={'mobile': mobile, 'email': email, 'credit_limit': credit}
            )
        ...
        if Invoice.objects.count() < 5:
            for i in range(15):
                ...
                subtotal = Decimal('0')
                tax_total = Decimal('0')
                selected = random.sample(prod_objs, random.randint(2, 5))
                for prod in selected:
                    qty = Decimal(str(random.randint(1, 4)))
                    price = prod.selling_price
                    gst_pct = prod.gst_percent
                    gst_amt = (price * qty * gst_pct / 100).quantize(Decimal('0.01'))
                ...
                invoice.save()
                Payment.objects.create(invoice=invoice, method=invoice.payment_method, amount=grand_total)
                # Use update to bypass auto_now
                Invoice.objects.filter(pk=invoice.pk).update(created_at=inv_date)
