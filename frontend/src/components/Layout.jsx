  ...
  return (
    <div ref={ref} className="relative">
      <div className={`flex items-center gap-2 rounded-xl px-3.5 py-2 w-full max-w-md transition-all duration-200 border ${
        open || q
          ? 'bg-white dark:bg-slate-800 border-blue-400 shadow-sm ring-2 ring-blue-100 dark:ring-blue-900'
          : 'bg-[var(--surface-elevated)] dark:bg-slate-800/60 border-[var(--line)] dark:border-slate-700 hover:border-[var(--primary-border)] dark:hover:border-slate-600'
      }`}>
      ...
      {open && q && (
        <div className="absolute top-full mt-2 left-0 w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-[var(--shadow-lg)] border border-slate-200 dark:border-slate-700 z-50 overflow-hidden">
          {searching ? (
            ...
            results.map(result => (
              <button key={`${result.type}-${result.id}`} onClick={() => { navigate(result.route); setQ(''); setOpen(false) }} className="w-full px-4 py-2.5 text-left hover:bg-slate-50 border-b border-slate-50 last:border-0 transition-colors">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-semibold text-slate-800 truncate">{result.label}</span>
                  <span className="text-[10px] uppercase text-slate-400 shrink-0 bg-slate-100 px-1.5 py-0.5 rounded">{result.type}</span>
                </div>
            ...
            <div className="px-4 py-5 text-center">
              <p className="text-xs font-medium text-slate-600">No results for "{q}"</p>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden">
          ...
          <div className="divide-y divide-slate-50 max-h-72 overflow-y-auto">
            ...
            {!hasNotifications && <div className="px-4 py-6 text-xs text-slate-500 text-center">All caught up — no action required</div>}
          </div>
        </div>
      )}
    </div>
    ...
    <ShopContext.Provider value={{ logoSrc, shopName }}>
      <div className="flex h-screen overflow-hidden" style={{ background: '#f1f5f9' }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          <header className="flex-shrink-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-[0_1px_0_rgba(15,23,42,0.06)]">
